...
start(ID,CPU,true) ->
   unregister(ID),
   register  (ID,spawn(f18X,run,[CPU]));
...
loop({run,CPU}) ->
   receive
      reset ->
         log:info(?TAG,"RESET"),
         trace:trace(f18X,{ CPU#cpu.id,reset}),     
...
exec(CPU,nop) ->
   log:info(?TAG,"NOP"),     
   trace:trace(f18X,{ CPU#cpu.id,nop }),     
   PC  = CPU#cpu.pc + 1,
   {ok,CPU#cpu{pc=PC}};

exec(CPU,read) ->
   log:info   (?TAG,"READ"),
   trace:trace(f18X,{ CPU#cpu.id,read}),     
   read(CPU);

exec(CPU,{write,Word}) ->
   log:info   (?TAG,"WRITE"),
   trace:trace(f18X,{ CPU#cpu.id,write,Word }),     
   write(CPU,Word);
...
write_wait(CPU) ->
   ...
   receive
      { Ch,read,ok } -> 
         trace:trace(f18X,{ CPU#cpu.id,write,ok }),     
         PC = CPU#cpu.pc + 1,
         {ok,CPU#cpu{pc = PC }};
